...
- Use preview to validate layout, timing, captions, and styling first.
- Keep fixing issues in preview until the user is satisfied; do not render intermediate versions that still need review.
- Treat rendering as the final step after preview is clean.
...
## Render Approval Rule

Never render during exploratory or fixup work. Finish preview-based review and fixes first, then render only with user approval or an explicit final-render request.
